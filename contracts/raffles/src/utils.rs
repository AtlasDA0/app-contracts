...
use utils::state::VanillaAssetInfo;
#[cfg(feature = "sg")]
use {
    crate::state_sg::{
    ...
    sg_std::CosmosMsg,
    utils::state::{into_sg_msg, AssetInfo, Response},
};
#[cfg(feature = "vanilla")]
use {
    ...
    raffle_info
        ...
        .map(|asset| match asset {
            AssetInfo::Cw721Coin(nft) => {
                ...
                into_sg_msg(message, nft.address.clone(), None)
            }
            AssetInfo::Sg721Token(sg721_token) => {
                ...
                into_sg_msg(message, sg721_token.address.clone(), None)
            }
...
pub fn ticket_cost(
    raffle_info: RaffleInfo,
    ticket_count: u32,
) -> Result<VanillaAssetInfo, ContractError> {
    // enforces only Coin is a ticket cost currently.
    Ok(match raffle_info.raffle_ticket_price {
        AssetInfo::Coin(x) => VanillaAssetInfo::Coin(Coin {
            denom: x.denom,
            amount: Uint128::from(ticket_count) * x.amount,
        }),
        _ => return Err(ContractError::WrongAssetType {}),
